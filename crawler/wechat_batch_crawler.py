    ...
    def fetch_and_save_article(self, article_info: Dict, account_info: Dict) -> bool:
        ...
        try:
            ...
            with open(filepath, 'w', encoding='utf-8') as f:
                ...
                f.write(f"机构: {institution}\n")
                f.write(f"公众号: {article_info['account_name']}\n")
                f.write(f"日期: {date_str}\n")
                f.write(f"链接: {article_info['link']}\n")
                f.write(f"分类: {category} - {content_type}\n")
                f.write(f"阅读数: {article_info.get('read_num', 0)}\n")
                f.write(f"点赞数: {article_info.get('like_num', 0)}\n")
                f.write("-" * 80 + "\n\n")
                f.write(content)
